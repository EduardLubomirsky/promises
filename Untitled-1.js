...
function startRecursive() {
    ...
    if (!startRecursive.sum) {
        startRecursive.sum = [];
    }

    return new Promise((resolve, reject) => {
        const partOfUsers = users.splice(0, 10);
        const usersSumPromises = [];
        partOfUsers.forEach(x => {
            usersSumPromises.push(processUser(x.id));
        });

        Promise.all(usersSumPromises).then((res) => {
            const sum = getSum(res);
